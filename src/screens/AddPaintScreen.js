...
const PageContent = ({navigation}) =>{
    ...
    const validateformData=()=>{
        if(formData.branch!="SELECT BRANCH" && formData.quantity!="" && formData.amount!="" && formData.supplier.length>2){
            if(formData.url){
            ...
            function saveStockData(filePath){
                getNetworkStatus((socket,url)=>{
                    socket.emit("saveStock",formData.description,formData.amount,formData.quantity,formData.supplier,formData.size,formData.branch,filePath,(cb)=>{
                        if(cb){
                ...
                <ScrollView style={{padding:10}}>
                    <View style={{margin:5,backgroundColor:'#e8e9f5',padding:5,borderRadius:10}}>
                        <View style={{padding:5,backgroundColor:'#fff',borderRadius:10}}>
                            <AisInput attr={{field:'description',icon:{name:'list',type:'Feather',min:5,color:'#5586cc'},keyboardType:null,placeholder:'Paint Description',color:'#009387',handleChange}} />
                            <AisInput attr={{field:'amount',icon:{name:'plus-circle',type:'Feather',min:5,color:'#5586cc'},keyboardType:"numeric",placeholder:'Enter Amount',color:'#009387',handleChange}} />
                            <AisInput attr={{field:'quantity',icon:{name:'trello',type:'Feather',min:5,color:'#5586cc'},keyboardType:"numeric",placeholder:'Enter Quantity',color:'#009387',handleChange}} />
                            <AisInput attr={{field:'size',icon:{name:'list',type:'FontAwesome',min:5,color:'#5586cc'},keyboardType:null,placeholder:'Enter Size',color:'#009387',handleChange}} />
